...
class Solve {
	...
	public void solve() {
		/*
		 * String[] weekd={"sun","mon","tue","wed","thu","fri","sat"}; String[]
		 * mont={"jan","feb","mar","apr","may","jun","jul","aug","sep","oct",
		 * "nov","dec"}; for(int i=0;i<weekd.length;i++){ map.put(weekd[i],i); }
		 * for(int i=0;i<mont.length;i++){ map.put(mont[i],i+1); } for(int
		 * i=0;i<61;i++){ map.put(""+i, i); }
		 */
		...
		// System.out.println(temp[1]+startCal+" "+temp[2]+
		// endCal);
				...
				for (int iday : sday) {
					for (int ihour : shour) {
						for (int iminute : sminute) {
							...
							// oneday.get(Calendar.DAY_OF_WEEK));
